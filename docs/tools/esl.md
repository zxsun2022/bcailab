...
ESL Reading now uses the canonical route `/reading`. Legacy `/esl` and `/esl/reading*` URLs redirect to the same experience with HTTP `308` so old non-GET requests still preserve their method.
