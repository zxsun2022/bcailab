...
is the one homepage link that still opens the popup directly.

For signed-out visitors, the homepage and `/english` show a **free-access chip**
(`FreeAccessChip`), "No account needed: … · Free to try first: …". It is generated by
`freeEntryPoints()` from the same registry and links to each module's signed-out destination,
so it cannot drift from the cards. `/english` cards also carry a per-card access tag, and its
hero leads with "Start dictation — no account", with Sign in as the secondary action. `/english` is public and serves
