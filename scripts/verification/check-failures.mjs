...
const probes = [
  ...
  ['doc-role', 'web', 'docs', 'docs/README.md', name]
];
const cases = selected.length ? probes.filter(([label]) => selected.includes(label)) : probes;
if (selected.some(label => !probes.some(([known]) => label === known))) throw new Error('Unknown failure probe');
console.log(`Failure-injection logs: ${logs}`);
try {
  for (const [label, scope, step, file, contents] of cases) {
    const path = resolve(root, file);
    const append = label === 'doc-link' || label === 'doc-role';
    if (!append && existsSync(path)) throw new Error(`Refusing to overwrite ${file}`);
    const original = append ? readFileSync(path, 'utf8') : null;
    const injected = label === 'doc-role'
      ? original.replace('**Document role:** current-guidance.', `**Document role:** ${name}.`)
      : (original ?? '') + contents;
    if (label === 'doc-role' && injected === original) throw new Error('Expected current-guidance marker before doc-role probe');
    ...
    try {
      ...
      if (result.error || result.status === 0 || !output.includes(`FAIL ${scope}: ${step}`) || !output.includes(label === 'doc-role' ? 'Invalid or missing document role in docs/README.md' : name)) {
  ...
  console.log(`PASS failure injection: ${cases.length} expected failures; all probe edits restored.`);
} catch (error) {
