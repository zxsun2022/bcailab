 ...
 *   {"type":"done","remainingToday":n}         success terminator
    ...
    async start(controller) {
      ...
      try {
        for await (const chunk of streamTranslateText({
          ...
          if (chunk.type === "delta") chars += chunk.text.length;
          controller.enqueue(frame(chunk));
        }
        // Charged only on a completed stream, matching the non-streaming path: a request
        // that failed part-way through does not consume the user's daily allowance.
        await recordTranslateUsage(context.env.DB, {
          ...prepared.identity,
          chars: prepared.text.length
        });
        controller.enqueue(
          frame({ type: "done", remainingToday: Math.max(0, prepared.remainingToday - 1) })
        );
      } catch (error) {
        console.error("translate stream failed", error, { chars });
        controller.enqueue(
