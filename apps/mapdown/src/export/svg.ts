...
function renderNode(theme: MindMapTheme, box: NodeBox): string {
  ...
  const rect =
    ...
    `height="${round(box.height)}" rx="${tokens.radius}" fill="${tokens.background}" ` +
  ...
  const text =
    ...
    `font-weight="${weight}" fill="${tokens.text}" dominant-baseline="central">${tspans}</text>`;
...
export function exportSvg(
  ...
  const nodes = result.order.map((id) => renderNode(theme, result.boxes[id]!)).join("");
